...
export async function crearActivo(prevState, formData) {
  ...
  const fecha_adquisicion = formData.get('fecha_compra')?.toString()
  ...
  if (!fecha_adquisicion) return { error: 'La fecha de compra es requerida' }
...
export async function actualizarActivo(prevState, formData) {
  ...
  const { error } = await supabaseAdmin
    .from('activos_fijos')
    .update({ nombre, categoria, valor_adquisicion, fecha_adquisicion, vida_util_anos })
    .eq('id', id)
